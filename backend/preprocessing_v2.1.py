...
def preprocess_all():
    ...
    for raw_day, file_suffix in day_types.items():  # file_suffix(파일 접미사) -> 저장할 파일명에 들어갈 요일 구분자
        ...
        for station_code, group in day_df.groupby('역사코드'):
            ...
            # iterrows() -> DataFrame의 각 행을 순회하며 (인덱스, 행의 데이터) 튜플 반환
    ...
    # 현재 시간표(df)에서 중복을 제외하고[.unique()] 실존 노선 번호 가져오기
    ...
    for _, row in df_trans.iterrows():
        ...
        if to_line in valid_lines:
            # 환승 노선이 현재 시간표에 존재하는 경우에만 처리. 그냥 환승시간 넣어도 되지만
            # 의미 없는 환승 노선 데이터를 줄여서 최적화하기 위함.
            walk_sec = time_str_to_seconds(row['환승소요시간'])
            walk_distance = int(row.get('환승거리', 0))
            if st_name not in transfer_dict:
                # 역사명으로 정리하면 같은 이름 다른 코드의 역이 섞일 수 있고
                # 최적 경로 탐색 시 과정이 많아짐.
                # v3에서는 역사코드 기준으로 바꾸었음.
                transfer_dict[st_name] = {}
            
            # Key 예시: "1:4" (1호선에서 4호선으로 갈 때)
            transfer_dict[st_name][f"{from_line}:{to_line}"] = {
                "walk_sec": walk_sec,
